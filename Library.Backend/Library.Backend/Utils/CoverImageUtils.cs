...
public class CoverImageUtils
{
    public static async Task ValidateCoverImage(IFormFile file)
    {
        ...
        {
            throw new CustomException("File size is too big, maximum is " + maxFileSize, StatusCodes.Status400BadRequest);
        }
        ...
        {
            throw new CustomException("Invalid file format. Only JPG, JPEG, PNG, and BMP are allowed.", StatusCodes.Status400BadRequest);
        }
        
        using var image = await Task.Run(() => Image.Load(file.OpenReadStream()));
        if (image.Height <= image.Width)
        {
            throw new CustomException("The image height must be greater than its width.", StatusCodes.Status400BadRequest);
        }
    }
